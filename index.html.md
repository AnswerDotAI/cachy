...
`no_cachy` sends the requests in its block live. It reads nothing from
the cache and writes nothing to it. `cachy` enables caching for one
block and restores the previous state on exit. It takes the same
arguments as `enable_cachy`.
