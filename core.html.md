...
with fresh ids then still hits the cache. The patterns apply to the URL
as well as the body, so a pattern over the port lets a test replay
against whichever local port serves. Only the key is affected; responses
are stored as-is (see `resp_norm_pats` above for the stored side).
...
A port in the URL is the same kind of ephemeral detail: two requests to
different local ports key alike once a pattern maps the port, and the
pattern comes off again so the rest of the page keys as before:
...
Some APIs return a different representation of the same URL depending on
the `Accept` header. GitHub returns a pull request as JSON by default,
and as a diff for `application/vnd.github.v3.diff`. `_key` includes
`Accept`, which gives the two requests separate cache entries:

``` python
u = 'https://api.github.com/repos/fastai/ghapi/pulls/205'
r1,r2 = [httpx.Request('GET', u, headers={'Accept': a}) for a in ('application/json', 'application/vnd.github.v3.diff')]
test_ne(_key(r1), _key(r2))
```
...
*Send requests live within the block, without reading or writing the
cache*

Some requests must reach the server while cachy is enabled. A test that
creates a resource and then lists it sends the same request twice and
expects two different responses. `no_cachy` sends every request in its
block live. It reads nothing from `cachy.jsonl` and writes nothing to
it.

`no_cachy` leaves the patches installed. It sets the `_off` flag, which
`_check` reads before anything else. httpbingo echoes a unique timestamp
header, which makes a live response differ from the cached one. The
cached record is still there after the block:

``` python
cached = httpx.post('https://httpbingo.org/post', json={'a':1})
with no_cachy(): live = httpx.post('https://httpbingo.org/post', json={'a':1})
test_ne(live.text, cached.text)
test_eq(httpx.post('https://httpbingo.org/post', json={'a':1}).text, cached.text)
```

`_off` is a `ContextVar`. The bypass applies to the current task and to
tasks created inside the block. Other tasks keep using the cache.

``` python
async def _post():
    async with AsyncClient() as c: return (await c.post('https://httpbingo.org/post', json={'a':2})).text
with no_cachy(): a,b = await asyncio.gather(_post(), _post())
test_ne(a, b)
```
...
*Enable cachy within the block, then restore the previous state*

`cachy` takes the same arguments as `enable_cachy`. It patches the
client classes as `enable_cachy` does, and the block applies to the
whole process. On exit it restores the configuration that was active
before the block. Here the outer configuration has `debug=True`. The
block runs quietly, and the request after it prints its hit again:

``` python
with cachy(debug=False): r = httpx.post('https://httpbingo.org/post', json={'a':1})
r = httpx.post('https://httpbingo.org/post', json={'a':1})
```
...
When cachy was disabled before the block, `cachy` disables it again on
exit:

``` python
with cachy(): r1,r2 = [httpx.post('https://httpbingo.org/post', json={'a':1}) for _ in range(2)]
test_eq(r1.text, r2.text)
test_ne(httpx.post('https://httpbingo.org/post', json={'a':1}).text, r1.text)
```
