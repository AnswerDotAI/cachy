...
The `requests` patch covers any `requests.Session` (httpbin echoes a
unique `X-Amzn-Trace-Id` per real call, so equal bodies prove the second
response came from cache).

``` python
r1 = requests.post('https://httpbin.org/post', json={'a': 'req'})
r1
```

``` python
r2 = requests.post('https://httpbin.org/post', json={'a': 'req'})
test_eq(r1.text, r2.text)
test_eq(r2.headers['content-type'], 'application/json')
```
